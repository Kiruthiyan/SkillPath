...
export default function RoadmapPage() {
  ...
  // Auto-generate ONLY when arriving with a specific courseId in the URL. Guarded by a ref
  // (rather than omitting profile from deps) so profile.stream/zscore are current at the time
  // of generation without re-triggering generation every time profile data changes.
  const autoGeneratedForRef = useRef<string | null>(null);
  useEffect(() => {
    if (courseIdParam && autoGeneratedForRef.current !== courseIdParam) {
      autoGeneratedForRef.current = courseIdParam;
  ...
  }, [courseIdParam, profile.stream, profile.zscore, generateRoadmap]);
