...
| 14 | `api-zod` duplicate export typecheck error | **Fixed** | [`packages/api-zod/src/index.ts`](packages/api-zod/src/index.ts) |
...
- `packages/api-spec/openapi.yaml` — `handbookYear` on `DashboardStats`
- `.env.example` — `CORS_ORIGINS`, `AI_REQUIRE_AUTH`
